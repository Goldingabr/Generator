...
def generate_task1():
    ...
    # Длина строки всегда 7 или 8 цифр
    length = random.randint(7, 8)
    
    # Сначала генерируем строку без ограничений
    result = ''.join(random.choice('0123') for _ in range(length))
    
    # Проверяем наличие более 2 нулей подряд и исправляем при необходимости
    while '000' in result:
        # Находим позицию трех нулей подряд
        pos = result.find('000')
        # Заменяем третий ноль случайной ненулевой цифрой
        replacement = random.choice('123')
        # Заменяем в строке
        result = result[:pos+2] + replacement + result[pos+3:]
    
    # Убеждаемся, что первая цифра не ноль
    if result[0] == '0':
